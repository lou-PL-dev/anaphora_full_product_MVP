...
to the user's `ideal_partner` Blueprint (pgvector cosine similarity over
OpenAI embeddings), then generates a grounded compatibility blurb per
candidate (`chains/matching_chain.py`). Candidates are NOT real users —
they're generated by `rag_demo/generate_personas.py`'s self-profile mode
and seeded via `rag_demo/ingest_candidates.py` (see that file and
