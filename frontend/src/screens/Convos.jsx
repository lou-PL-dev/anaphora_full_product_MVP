  ...
  return (
    <div className="ap-screen" style={{ flex: 1, minHeight: 0, overflowY: 'auto', background: 'linear-gradient(180deg, #F2EDE6 0%, #FFFFFF 42%, #F2EDE6 100%)', padding: '64px 22px 24px' }}>
      ...
      <button
        ...
        style={{ marginTop: 22, width: '100%', textAlign: 'left', padding: '24px 22px', borderRadius: 24, border: 'none', background: '#A69ACD', cursor: 'pointer', boxShadow: '0 12px 30px rgba(166,154,205,.22)' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14 }}>
          <div style={{ fontSize: 10, letterSpacing: '.15em', color: '#F2EDE6' }}>YOUR CONVERSATION</div>
          <div style={{ padding: '6px 10px', borderRadius: 999, background: '#F2EDE6', color: '#2F4A3F', fontSize: 10.5, fontWeight: 600 }}>{convoCta} →</div>
        </div>
        ...
        <div style={{ marginTop: 8, maxWidth: 300, fontSize: 12.5, lineHeight: 1.6, color: '#FFFFFF' }}>Keep adding nuance about who you're looking for — and about you.</div>
      ...
      <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 10 }}>
        {discoveries.map((d) => (
          <button
            ...
            style={{ width: '100%', textAlign: 'left', padding: 18, borderRadius: 20, border: `1px solid ${d.done ? '#A69ACD' : '#DDEAE6'}`, background: d.done ? 'linear-gradient(145deg, #F2EDE6 0%, #FFFFFF 100%)' : '#FFFFFF', cursor: 'pointer' }}
          >
