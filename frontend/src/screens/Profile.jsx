...
export default function Profile({
  ...
  const genderOptions = ['Women', 'Men', 'Everyone'].map((g) => ({
    ...
    border: gender === g ? LAV : '#DDEAE6',
    bg: gender === g ? 'rgba(166,154,205,.12)' : '#FFFFFF',
    fg: gender === g ? SAGE : '#2F4A3F',
  }));
  ...
  const breakdown = [
    ...
    key, title, weight,
    ring: breakdownMet[key] ? SAGE : '#DDEAE6',
  ...
  return (
    <div className="ap-screen" style={{ flex: 1, minHeight: 0, overflowY: 'auto', background: '#FFFFFF', padding: '64px 22px 26px' }}>
      ...
      <div style={{ marginTop: 8, fontSize: 12.5, color: '#A69ACD' }}>Anonymous session · your data stays yours</div>

      <div style={{ marginTop: 24, padding: 20, borderRadius: 20, background: '#FFFFFF', border: '1px solid #DDEAE6' }}>
        <div style={{ fontSize: 11, letterSpacing: '.14em', color: '#2F4A3F' }}>BASIC MATCHING PREFERENCES</div>
        <div style={{ marginTop: 16, fontSize: 12.5, color: '#2F4A3F' }}>Looking to meet</div>
        ...
        <button onClick={onSavePreferences} disabled={!gender || preferencesSaving} style={{ marginTop: 16, width: '100%', padding: '12px 16px', borderRadius: 999, border: 'none', background: gender && !preferencesSaving ? SAGE : '#F2EDE6', color: gender && !preferencesSaving ? '#FFFFFF' : SAGE, fontSize: 13, cursor: gender && !preferencesSaving ? 'pointer' : 'default' }}>{preferencesSaving ? 'Saving…' : 'Save preferences'}</button>
      </div>
      ...
      {readiness < 100 && (
        <div style={{ marginTop: 14, padding: 20, borderRadius: 20, background: '#F2EDE6' }}>
          ...
          <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 11 }}>
            {breakdown.map((b) => <div key={b.key} style={{ display: 'flex', alignItems: 'center', gap: 10 }}><span style={{ flex: 'none', width: 16, height: 16, borderRadius: '50%', border: `1.5px solid ${b.ring}`, background: b.fill }} /><span style={{ flex: 1, fontSize: 12.5, color: '#2F4A3F' }}>{b.title}</span><span style={{ flex: 'none', fontSize: 11.5, color: '#A69ACD' }}>{b.weight}%</span></div>)}
          </div>
        </div>
      )}
      ...
      <button onClick={openPlans} style={{ marginTop: 14, width: '100%', textAlign: 'left', padding: 20, borderRadius: 20, border: 'none', background: 'linear-gradient(140deg, rgba(166,154,205,.18), #DDEAE6)', cursor: 'pointer' }}>
        ...
        <div style={{ marginTop: 6, fontSize: 12.5, color: '#2F4A3F' }}>More Discoveries, deeper match explanations.</div>
      </button>

      <div style={{ marginTop: 22, textAlign: 'center', fontSize: 11, color: '#A69ACD', letterSpacing: '.02em' }}>
