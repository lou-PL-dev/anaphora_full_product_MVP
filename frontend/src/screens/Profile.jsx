    ...
    <div className="ap-screen" style={{ flex: 1, minHeight: 0, overflowY: 'auto', background: '#FBF9F6', padding: '64px 22px 26px' }}>
      ...
      <div style={{ marginTop: 24, padding: 20, borderRadius: 20, background: '#FFFFFF', border: '1px solid rgba(47,74,63,.08)' }}>
        ...
        <button
          ...
          disabled={!gender || preferencesSaving}
          style={{ marginTop: 16, width: '100%', padding: '12px 16px', borderRadius: 999, border: 'none', background: gender && !preferencesSaving ? SAGE : 'rgba(47,74,63,.28)', color: '#FFFFFF', fontSize: 13, cursor: gender && !preferencesSaving ? 'pointer' : 'default' }}
        >
      ...
      <div style={{ marginTop: 22, textAlign: 'center', fontSize: 11, color: '#94A09A', letterSpacing: '.02em' }}>
        <button onClick={goPrivacy} style={{ background: 'none', border: 'none', padding: 0, font: 'inherit', letterSpacing: 'inherit', color: LAV, textDecoration: 'underline', cursor: 'pointer' }}>Privacy by design</button>
        {' · '}
        <button onClick={goTerms} style={{ background: 'none', border: 'none', padding: 0, font: 'inherit', letterSpacing: 'inherit', color: LAV, textDecoration: 'underline', cursor: 'pointer' }}>The fine print</button>
      </div>
    </div>
  );
}
