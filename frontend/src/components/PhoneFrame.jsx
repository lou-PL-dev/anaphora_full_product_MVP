export default function PhoneFrame({ framed, onToggleFrame, modeLabel, modeDot, onResetAll, children }) {
  ...
  const innerStyle = framed
    ? { position: 'relative', width: '100%', height: '100%', borderRadius: 53, overflow: 'hidden', background: '#F2EDE6' }
    : { position: 'relative', width: '100%', height: '100%', overflow: 'hidden', background: '#F2EDE6' };
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(120% 90% at 20% 0%, #F6F2EC 0%, #EDE8E1 55%, #E3DDD5 100%)', fontFamily: 'Inter, system-ui, sans-serif', padding: 0 }}>
      ...
      <div style={outerStyle}>
        <div style={innerStyle}>
          ...
          <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', background: '#F2EDE6' }}>
            {children}
          </div>
        </div>
      </div>
