...
export default function TabBar({ activeScreen, onGo }) {
  return (
    <div style={{ flex: 'none', display: 'flex', padding: '10px 8px 26px', borderTop: '1px solid rgba(47,74,63,.08)', background: 'rgba(251,249,246,.96)', backdropFilter: 'blur(10px)' }}>
      {TABS.map(([key, label, icon]) => {
        const color = activeScreen === key ? SAGE : '#AEB8B2';
        return (
          <button
            key={key}
            onClick={() => onGo(key)}
