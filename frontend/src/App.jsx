...
export default function App() {
  ...
  useEffect(() => {
    ...
    api('GET', '/discovery/life_you_are_building').then((r) => { if (r && r.questions) patch({ questions: r.questions }); });
  ...
  const readinessCopy = readiness >= 90 ? ['Ready when you are', 'We know enough to look for people who actually fit.'] : readiness >= 60 ? ['Coming into focus', 'A little more and matches start making real sense.'] : readiness > 0 ? ['A good beginning', 'Every answer sharpens who we look for.'] : ['Nothing yet', 'One conversation is all it takes to start.'];
