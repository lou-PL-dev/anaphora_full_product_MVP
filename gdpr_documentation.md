...
| `blueprint_signals` | `perspective` (ME / IDEAL_PARTNER / US), `category`, `label`, `evidence_text`, `strength`, `confidence` | Structured personal-characteristic and preference data extracted from the conversation | Inferred/derived personal data — see §3 for special-category analysis |
...
| Rectification | **Supported** | `PATCH /blueprint/signal/{id}` lets a tester correct any individual signal's label/strength directly from the Blueprint screen |
